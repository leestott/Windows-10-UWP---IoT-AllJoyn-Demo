...
std::map<alljoyn_interfacedescription, WeakReference*> lightbulbConsumer::SourceInterfaces;

lightbulbConsumer::lightbulbConsumer(AllJoynBusAttachment^ busAttachment)
...
lightbulbConsumer::~lightbulbConsumer()
{
...
void lightbulbConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void lightbulbConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void lightbulbConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus lightbulbConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<lightbulbJoinSessionResult^>^ lightbulbConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ lightbulbWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> lightbulbJoinSessionResult^
    {
        auto result = ref new lightbulbJoinSessionResult();
        result->Status = AllJoynStatus::Ok;
        result->Consumer = nullptr;

        result->Consumer = ref new lightbulbConsumer(watcher->BusAttachment);
...
IAsyncOperation<lightbulbSwitchResult^>^ lightbulbConsumer::SwitchAsync(_In_ bool interfaceMemberOn)
{
    return create_async([this, interfaceMemberOn]() -> lightbulbSwitchResult^
    {
...
IAsyncOperation<lightbulbGetLocationResult^>^ lightbulbConsumer::GetLocationAsync()
{
    return create_async([this]() -> lightbulbGetLocationResult^
    {
...
IAsyncOperation<lightbulbGetStatusResult^>^ lightbulbConsumer::GetStatusAsync()
{
    return create_async([this]() -> lightbulbGetStatusResult^
    {
...
void lightbulbConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
...
int32 lightbulbConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<lightbulbConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<lightbulbConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<lightbulbConsumer>
    };
    ...
    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "com.taulty.lightbulb");
