...
lightbulbWatcher::lightbulbWatcher(AllJoynBusAttachment^ busAttachment) :
...
void lightbulbWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.taulty.lightbulb" };
...
void lightbulbWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.taulty.lightbulb"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.taulty.lightbulb"),
            port);
        Added(this, args);
    }
...
void lightbulbWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<lightbulbWatcher>
        };
...
void lightbulbWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_lightbulbIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &lightbulbWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void lightbulbWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void lightbulbWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
